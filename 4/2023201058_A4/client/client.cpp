...
bool isloggedin, isfilecorrupt;
string logfile, t1ip, t2ip, clientip;
...
unordered_map<string, string> downloading_file_list;
unordered_map<string, string> downloaded_file_list;
...
void add_to_downloading_file_list(string i1, string i2)
{
    downloading_file_list.insert({i1, i2});
}
void remove_from_downloading_file_list(string i1)
{
    downloading_file_list.erase(i1);
}
void add_to_downloaded_file_list(string i1, string i2)
{
    downloaded_file_list.insert({i1, i2});
}
...
int chunkwriterfunction(int client_socket, long long int chunkNum, char *fpath)
{
    char buffer[524288];
    string cont = "";
    int n;
    ...
    {
        n = read(client_socket, buffer, 524287);
        if (n <= 0)
        {
            break;
        }
...
string client_as_server_connect(char *cserverip, char *cserverport_ip, string cmd)
{
    ...
    if (connect(client_socket, (struct sockaddr *)&caddress, sizeof(caddress)) < 0)
    {
    ...
    {
        char sreply[10240] = {0};
        int sstatus = send(client_socket, &cmd[0], strlen(&cmd[0]), MSG_NOSIGNAL);
        if (sstatus == -1)
        {
        ...
        int rstatus = read(client_socket, sreply, 10240);
        if (rstatus < 0)
        {
            perror("err: ");
            return "error";
    ...
    if (ccomand == "get_chunk")
    {
        ...
        else
        {
            ...
            chunkwriterfunction(client_socket, chunkNum, &despath[0]);
    ...
    if (ccomand == "get_file_path")
    {
        ...
        if (read(client_socket, sreply, 10240) < 0)
        {
            perror("err: ");
            return "error";
        }
...
void sending_chuck_function(char *fpath, int chunknumber, int client_socket)
{
    string sent = "";
    ...
    int rc = send(client_socket, buffer, count, 0);
...
void client_request_handler(int client_socket)
{
    string clientid = "";
    char ipline[1024] = {0};
    if (read(client_socket, ipline, 1024) <= 0)
    {
        close(client_socket);
        return;
    }
    ...
    else if (inputvect[0] == "get_file_path")
    {
        ...
        write(client_socket, &fpath[0], strlen(fpath.c_str()));
        close(client_socket);
        return;
    }
    else if (inputvect[0] == "get_chunk")
    {
        ...
        sending_chuck_function(&fpath[0], chunknumber, client_socket);
        close(client_socket);
        return;
    }
...
void *client_as_server(void *arg)
{
    ...
    int ipton = inet_pton(AF_INET, &clientip[0], &sock_address.sin_addr);
    if (ipton <= 0)
    {
    ...
    while (true)
    {
        int client_socket = accept(server_sock, (struct sockaddr *)&sock_address, (socklen_t *)&(sock_address));
        if (client_socket < 0)
        {
            perror("Acceptance error");
        }

        threadsvect.push_back(thread(client_request_handler, client_socket));
    }
...
int already_uploaded(int sock, string fname, vector<string> input)
{
...
void append_ip_port(string &fileDetails, string fpathstr, string filehash, string piece_wise_hash, string fsize)
{
...
void piece_wise_algo(vector<string> clients, vector<string> inpt)
{
    ...
    if (fp == 0)
    {
        ...
        string clientfpath;
        add_to_downloading_file_list(inpt[2], inpt[1]);
        ...
        remove_from_downloading_file_list(inpt[2]);
        add_to_downloaded_file_list(inpt[2], inpt[1]);
...
int processcommand(int sock, vector<string> inputvect)
{
    ...
    if (inputvect[0] == "download_file")
    {
        ...
        if (sreply1 == "DirectoryNotPresent")
        {
    ...
    if (inputvect[0] == "upload_file")
    {
        ...
        if (inputvect.size() == 3)
        {
            ...
            string fileDetails = "";
            int check_already_uploaded = already_uploaded(sock, fname, inputvect);
            ...
            if (piece_wise_hash != "$")
            {
                ...
                append_ip_port(fileDetails, fpathstr, filehash, piece_wise_hash, fsize);
    ...
    {
        for (auto it : downloading_file_list)
        {
            cout << "[D] " << it.second << " " << it.first << endl;
        }
        for (auto it : downloaded_file_list)
        {
